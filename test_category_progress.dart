...
void main() async {
  ...
  try {
    ...
    for (final category in ExerciseCategory.values) {
      ...
      if (count > 0) {
        print(
            '${category.name}: $count exercises, ${volume.toStringAsFixed(1)} total volume');
      }
    }
    ...
    for (final category in mainCategories) {
      ...
      final percentage =
          providerTotal > 0 ? (volume / providerTotal * 100) : 0.0;
      print(
          '${category.name}: ${percentage.toStringAsFixed(1)}% (from provider)');
    }
