...
void main() async {
  ...
  try {
    ...
    if (allPerformances.isEmpty) {
      print('   ❌ NO PERFORMANCES FOUND - User needs to complete exercises first');
      container.dispose();
      return;
    }
    ...
    for (int i = 0; i < 3 && i < allPerformances.length; i++) {
      ...
      if (perf.sets.isNotEmpty) {
        ...
        print('     First set: ${firstSet.reps} reps × ${firstSet.weight ?? 0} kg');
      }
    }
    ...
    for (final perf in allPerformances) {
      ...
      if (exercise == null) {
        noMatchCount++;
        if (noMatchCount <= 3) {
          print('   ⚠️  No match for exerciseId: ${perf.exerciseId} (${perf.exerciseName})');
        }
        continue;
      }
    ...
    for (final category in ExerciseCategory.values) {
      ...
      if (count > 0) {
        print('   ${category.name.padRight(15)}: $count exercises, ${volume.toStringAsFixed(1)} volume');
      }
    }
    ...
    for (final category in mainCategories) {
      final volume = categoryVolumes[category] ?? 0.0;
      final percentage = totalMainVolume > 0 ? (volume / totalMainVolume * 100) : 20.0;
      print('   ${category.name.padRight(15)}: ${percentage.toStringAsFixed(1)}%');
    }
    ...
    for (final category in mainCategories) {
      ...
      final percentage = providerTotal > 0 ? (volume / providerTotal * 100) : 20.0;
      print('   ${category.name.padRight(15)}: ${percentage.toStringAsFixed(1)}%');
    }
