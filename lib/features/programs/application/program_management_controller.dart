...
class ProgramManagementService {
  ...
  static Future<bool> stopCurrentProgram({
    ...
    try {
      _logger.i(
          'ProgramManagementService: Stopping current program with option: $option');
      ...
      _logger
          .d('ProgramManagementService: Active program ID: $activeProgramId');
      ...
      if (success) {
        _logger.i(
            'ProgramManagementService: Successfully stopped program with option: $option');

        // Small delay to ensure data is properly cleared
        await Future.delayed(const Duration(milliseconds: 100));

        // Invalidate providers in the correct order to prevent race conditions
        // First invalidate the core data providers
  ...
  static Future<int> getCurrentProgramProgressEventCount(WidgetRef ref) async {
    try {
      ...
      final events =
          await progressRepo.getByProgram(currentState!.activeProgramId!);
  ...
  static Future<bool> _stopAndDeleteProgress(
      WidgetRef ref, String programId) async {
    try {
      _logger.d(
          'ProgramManagementService: Stopping program and deleting progress for: $programId');
      ...
      if (success) {
        _logger.i(
            'ProgramManagementService: Successfully deleted progress, cleared state, and reset analytics');
      } else {
        _logger.e(
            'ProgramManagementService: Failed to delete progress, clear state, or reset analytics');
      }

      return success;
    } catch (e, stackTrace) {
      _logger.e(
          'ProgramManagementService: Error stopping and deleting progress',
          error: e,
  ...
  static Future<bool> _stopAndDeleteEverything(
      WidgetRef ref, String programId) async {
    try {
      _logger.d(
          'ProgramManagementService: Stopping program and deleting everything for: $programId');
      ...
      _logger.e(
          'ProgramManagementService: Error stopping and deleting everything',
          error: e,
          stackTrace: stackTrace);
      return false;
    }
