...
class MigrationService {
  ...
  static Future<void> ensureMigrations() async {
    try {
      ...
      _logger.i('MigrationService: Target schema version: $_currentSchemaVersion');
      
      if (currentVersion < _currentSchemaVersion) {
        _logger.i('MigrationService: Migrations needed from v$currentVersion to v$_currentSchemaVersion');
        await _runMigrations(currentVersion, _currentSchemaVersion);
      } else {
        _logger.i('MigrationService: Database is up to date, no migrations needed');
      }
  ...
  static Future<int> _getCurrentSchemaVersion() async {
    try {
      final versionString = await LocalKVStore.read(
        HiveBoxes.migrations, 
        _schemaVersionKey
      );
      
      if (versionString == null) {
        _logger.d('MigrationService: No schema version found, assuming first run (v0)');
        return 0;
      }
      ...
      _logger.w('MigrationService: Failed to read schema version, assuming v0', error: e);
      return 0;
    }
  }
  ...
  static Future<void> _runMigrations(int startVersion, int endVersion) async {
    try {
      _logger.i('MigrationService: Running migrations from v$startVersion to v$endVersion');
      ...
      for (int version = startVersion + 1; version <= endVersion; version++) {
        ...
        _logger.i('MigrationService: Successfully applied migration to v$version');
      }
      
      _logger.i('MigrationService: All migrations completed successfully');
      
    } catch (e, stackTrace) {
      _logger.e('MigrationService: Migration failed', error: e, stackTrace: stackTrace);
      rethrow;
    }
  }
  ...
  static Future<void> _applyMigration(int version) async {
    switch (version) {
      ...
      default:
        _logger.w('MigrationService: No migration defined for version $version');
    }
  }
  ...
  static Future<void> _migrateToV1() async {
    ...
    _logger.i('MigrationService: V1 migration complete - Initial schema established');
  }
  ...
  static Future<void> forceMigrationTo(int targetVersion) async {
    try {
      _logger.w('MigrationService: FORCING migration to version $targetVersion');
      
      final currentVersion = await _getCurrentSchemaVersion();
      if (targetVersion > currentVersion) {
        await _runMigrations(currentVersion, targetVersion);
      } else {
        _logger.w('MigrationService: Target version $targetVersion is not greater than current $currentVersion');
      }
      
    } catch (e, stackTrace) {
      _logger.e('MigrationService: Failed to force migration to version $targetVersion', 
                error: e, stackTrace: stackTrace);
      rethrow;
    }
  }
  ...
  static Future<void> resetMigrations() async {
    try {
      ...
      final success = await LocalKVStore.delete(
        HiveBoxes.migrations,
        _schemaVersionKey
      );
