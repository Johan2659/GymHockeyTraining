...
class SecureKeyService {
  ...
  static Future<Uint8List> getOrCreateEncryptionKey() async {
    try {
      ...
      _logger.d(
          'SecureKeyService: Secure storage returned key: ${existingKey != null ? '[KEY_PRESENT]' : 'null'}');

      if (existingKey != null && existingKey.isNotEmpty) {
        _logger.i(
            'SecureKeyService: Found existing encryption key in secure storage');
        final keyBytes = base64Decode(existingKey);

        if (keyBytes.length == 32) {
          _logger.i(
              'SecureKeyService: Using existing 32-byte encryption key from secure storage');
          return Uint8List.fromList(keyBytes);
        } else {
          _logger.w(
              'SecureKeyService: Existing key has wrong length (${keyBytes.length}), trying fallback');
        }
      } else {
        _logger.w(
            'SecureKeyService: No existing encryption key found in secure storage, trying fallback');
      }
      ...
      _logger.d(
          'SecureKeyService: Fallback storage returned key: ${fallbackKey != null ? '[KEY_PRESENT]' : 'null'}');

      if (fallbackKey != null && fallbackKey.isNotEmpty) {
        _logger.i(
            'SecureKeyService: Found existing encryption key in fallback storage');
        final keyBytes = base64Decode(fallbackKey);

        if (keyBytes.length == 32) {
          _logger.i(
              'SecureKeyService: Using existing 32-byte encryption key from fallback storage');
          ...
          try {
            ...
            _logger.d(
                'SecureKeyService: Successfully restored key to secure storage');
          } catch (e) {
            _logger.w(
                'SecureKeyService: Failed to restore key to secure storage: $e');
          }
          return Uint8List.fromList(keyBytes);
        } else {
          _logger.w(
              'SecureKeyService: Fallback key has wrong length (${keyBytes.length}), trying dev file');
        }
      } else {
        _logger.w(
            'SecureKeyService: No existing encryption key found in fallback storage, trying dev file');
      }
      ...
      try {
        ...
        if (devKey != null) {
          _logger.i(
              'SecureKeyService: Found existing encryption key in dev file storage');
          final keyBytes = base64Decode(devKey);

          if (keyBytes.length == 32) {
            _logger.i(
                'SecureKeyService: Using existing 32-byte encryption key from dev file');
            // Store in both secure and SharedPreferences for next time
            await _storeKeyInAllStorages(devKey);
            return Uint8List.fromList(keyBytes);
          } else {
            _logger.w(
                'SecureKeyService: Dev file key has wrong length (${keyBytes.length}), creating new');
          }
        } else {
          _logger.w(
              'SecureKeyService: No existing encryption key found in dev file storage');
        }
      } catch (e) {
      ...
      _logger.i(
          'SecureKeyService: Successfully created and stored new encryption key');
      return newKey;
    } catch (e, stackTrace) {
      _logger.e('SecureKeyService: Failed to get or create encryption key',
  ...
  static Future<void> _storeKeyInAllStorages(String keyBase64) async {
    ...
    try {
      ...
      _logger.i(
          'SecureKeyService: Stored key in secure storage: ${keyBase64.substring(0, 8)}...');
    } catch (e) {
    ...
    try {
      ...
      _logger.i(
          'SecureKeyService: Stored key in fallback storage: ${keyBase64.substring(0, 8)}...');
    } catch (e) {
      _logger
          .w('SecureKeyService: Failed to store key in fallback storage: $e');
    }
    ...
    try {
      ...
      _logger.i(
          'SecureKeyService: Stored key in dev file storage: ${keyBase64.substring(0, 8)}...');
    } catch (e) {
      _logger
          .w('SecureKeyService: Failed to store key in dev file storage: $e');
    }
  }
  ...
  static Future<String?> _getDevFileKey() async {
    try {
      ...
      if (await keyFile.exists()) {
        ...
        _logger.d(
            'SecureKeyService: Dev file key found: ${content.substring(0, 8)}...');
        return content.trim();
      } else {
  ...
  static Future<Uint8List> _generateSecureKey() async {
    try {
      ...
      final secureRandom = List<int>.generate(
          32, (_) => DateTime.now().millisecondsSinceEpoch.hashCode % 256);
  ...
  static Future<void> deleteEncryptionKey() async {
    try {
      ...
      try {
        ...
        _logger
            .w('SecureKeyService: Encryption key deleted from secure storage');
      } catch (e) {
        _logger.w(
            'SecureKeyService: Failed to delete key from secure storage: $e');
      }
      ...
      try {
        ...
        _logger.w(
            'SecureKeyService: Encryption key deleted from fallback storage');
      } catch (e) {
        _logger.w(
            'SecureKeyService: Failed to delete key from fallback storage: $e');
      }
    } catch (e, stackTrace) {
      _logger.e('SecureKeyService: Failed to delete encryption key',
          error: e, stackTrace: stackTrace);
      rethrow;
    }
  }
  ...
  static Future<bool> hasEncryptionKey() async {
    try {
      ...
      _logger.e('SecureKeyService: Failed to check for encryption key',
          error: e);
      return false;
    }
  }
