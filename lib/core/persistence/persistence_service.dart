...
class PersistenceService {
  ...
  static Future<void> initialize() async {
    ...
    try {
      _logger.i('PersistenceService: Initializing schema versioning and fallback...');
      
      // Initialize SharedPreferences for fallback (skip in tests)
      try {
        _sharedPrefs = await SharedPreferences.getInstance();
      } catch (e) {
        _logger.w('PersistenceService: SharedPreferences not available (test environment?), continuing with Hive only');
        _sharedPrefs = null;
      }
    ...
    try {
      ...
      if (_sharedPrefs != null) {
        ...
        if (fallbackValue != null) {
          _logger.w('PersistenceService: Using SharedPreferences fallback for $boxName/$key');
      ...
      if (_sharedPrefs != null) {
        try {
          ...
          final fallbackValue = _sharedPrefs!.getString(fallbackKey);
          if (fallbackValue != null) {
            _logger.w('PersistenceService: Emergency fallback successful for $boxName/$key');
            return fallbackValue;
          }
        } catch (fallbackError) {
          _logger.e('PersistenceService: Both Hive and SharedPreferences failed for $boxName/$key', 
                    error: fallbackError);
        }
      }
  ...
  static Future<bool> writeWithFallback(String boxName, String key, String value) async {
    ...
    try {
      ...
      _logger.w('PersistenceService: Hive write failed for $boxName/$key', error: e);
    }
    ...
    if (_sharedPrefs != null) {
      try {
        ...
        _logger.w('PersistenceService: SharedPreferences write failed for $boxName/$key', error: e);
      }
    }
    ...
    if (!success && _sharedPrefs != null) {
      _logger.e('PersistenceService: Both storage methods failed for $boxName/$key');
    } else if (!hiveSuccess && _sharedPrefs != null) {
      _logger.w('PersistenceService: Only SharedPreferences succeeded for $boxName/$key');
    } else if (!prefsSuccess && _sharedPrefs != null) {
      _logger.d('PersistenceService: Hive succeeded, SharedPreferences backup failed for $boxName/$key');
    }
  ...
  static Future<int> getSchemaVersion() async {
    try {
      // Try Hive first
      final hiveVersion = await LocalKVStore.read(HiveBoxes.settings, _schemaVersionKey);
      if (hiveVersion != null) {
      ...
      _logger.w('PersistenceService: Failed to get schema version, defaulting to 0');
      return 0;
    }
  }
  ...
  static Future<void> setSchemaVersion(int version) async {
    try {
      ...
      await LocalKVStore.write(HiveBoxes.settings, _schemaVersionKey, version.toString());
  ...
  static Future<void> _checkAndRunMigrations() async {
    ...
    if (currentVersion < _currentSchemaVersion) {
      _logger.i('PersistenceService: Running migrations from v$currentVersion to v$_currentSchemaVersion');
  ...
  static Future<void> _migrateToV1() async {
    try {
      _logger.d('PersistenceService: Running migration to v1 - backing up existing data');
  ...
  static Future<void> _backupCriticalData() async {
    ...
    for (final item in criticalKeys) {
      try {
        ...
        _logger.w('PersistenceService: Failed to backup ${item['box']}/${item['key']}', error: e);
      }
    }
  }
  ...
  static Future<bool> healthCheck() async {
    try {
      ...
      // Test write
      final writeSuccess = await writeWithFallback(HiveBoxes.settings, testKey, testValue);
