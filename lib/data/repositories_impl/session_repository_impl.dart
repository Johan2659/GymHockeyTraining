...
class SessionRepositoryImpl implements SessionRepository {
  ...
  Future<Session?> getById(String id) async {
    try {
      LoggerService.instance
          .debug('Getting session by ID: $id', source: 'SessionRepositoryImpl');
      ...
      if (session != null) {
        ...
        LoggerService.instance
            .warning('Session not found: $id', source: 'SessionRepositoryImpl');
  ...
  Future<List<Session>> getByProgramId(String programId) async {
    try {
      LoggerService.instance.debug('Getting sessions for program: $programId',
          source: 'SessionRepositoryImpl');
      _logger
          .d('SessionRepositoryImpl: Getting sessions for program: $programId');
      ...
      LoggerService.instance.info(
          ...
          source: 'SessionRepositoryImpl',
          metadata: {'programId': programId, 'sessionCount': sessions.length});
      _logger.i(
          'SessionRepositoryImpl: Found ${sessions.length} sessions for program $programId');
      return sessions;
    } catch (e, stackTrace) {
      ...
      _logger.e(
          'SessionRepositoryImpl: Failed to get sessions for program $programId',
          error: e,
          stackTrace: stackTrace);
      return [];
    }
  ...
  Future<List<Session>> getByWeek(String programId, int week) async {
    try {
      LoggerService.instance.debug(
          'Getting sessions for program $programId, week $week',
          source: 'SessionRepositoryImpl');
      _logger.d(
          'SessionRepositoryImpl: Getting sessions for program $programId, week $week');

      final sessions = await _localSource.getSessionsByWeek(programId, week);

      LoggerService.instance.info(
          'Found ${sessions.length} sessions for week $week',
          source: 'SessionRepositoryImpl',
          metadata: {
            'programId': programId,
            'week': week,
      ...
      _logger.i(
          'SessionRepositoryImpl: Found ${sessions.length} sessions for week $week');
      return sessions;
    } catch (e, stackTrace) {
