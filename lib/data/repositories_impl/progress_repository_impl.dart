...
class ProgressRepositoryImpl implements ProgressRepository {
  ...
  Future<List<ProgressEvent>> getByProgram(String programId) async {
    try {
      _logger.d('ProgressRepositoryImpl: Getting events for program: $programId');
      
      final events = await _localSource.getEventsByProgram(programId);
      
      _logger.i('ProgressRepositoryImpl: Found ${events.length} events for program $programId');
      return events;
      
    } catch (e, stackTrace) {
      _logger.e('ProgressRepositoryImpl: Failed to get events for program $programId', 
                error: e, stackTrace: stackTrace);
      return [];
    }
  }
  ...
  @override
  Future<List<ProgressEvent>> getByDateRange(DateTime start, DateTime end) async {
    try {
      ...
      _logger.i('ProgressRepositoryImpl: Found ${events.length} events in date range');
      return events;
