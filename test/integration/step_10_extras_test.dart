  ...
  group('Step 10 - Extras Implementation Tests', () {
    test('LocalExtrasSource loads all extras correctly', () async {
      ...
      final expressWorkouts =
          extras.where((e) => e.type == ExtraType.expressWorkout).toList();
      ...
      final bonusChallenges =
          extras.where((e) => e.type == ExtraType.bonusChallenge).toList();
      ...
      expect(
          bonusChallenges.any((e) => e.id == 'challenge_burpee_ladder'), true);

      // Check mobility & recovery
      final mobilityRecovery =
          extras.where((e) => e.type == ExtraType.mobilityRecovery).toList();
    ...
    test('ExtrasRepository filters by type correctly', () async {
      ...
      final expressWorkouts =
          await repository.getByType(ExtraType.expressWorkout);
      expect(expressWorkouts.length, 3);
      expect(expressWorkouts.every((e) => e.type == ExtraType.expressWorkout),
          true);

      // Test bonus challenges
      final bonusChallenges =
          await repository.getByType(ExtraType.bonusChallenge);
      expect(bonusChallenges.length, 3);
      expect(bonusChallenges.every((e) => e.type == ExtraType.bonusChallenge),
          true);

      // Test mobility & recovery
      final mobilityRecovery =
          await repository.getByType(ExtraType.mobilityRecovery);
      expect(mobilityRecovery.length, 3);
      expect(
          mobilityRecovery.every((e) => e.type == ExtraType.mobilityRecovery),
          true);
    });
    ...
    test('ExtraItem model has correct properties', () async {
      ...
      expect(cardioBlast.title, '15-Min Cardio Blast');
      expect(cardioBlast.description,
          'High-intensity cardio circuit to boost endurance and agility');
      expect(cardioBlast.type, ExtraType.expressWorkout);
      expect(cardioBlast.xpReward, 50);
      expect(cardioBlast.duration, 15);
    ...
    test('Bonus challenges have higher XP rewards', () async {
      final source = LocalExtrasSource();
      final bonusChallenges =
          await source.getExtrasByType(ExtraType.bonusChallenge);

      // All bonus challenges should have XP rewards >= 80
      expect(bonusChallenges.every((e) => e.xpReward >= 80), true);

      // Check specific challenges
      final pushupChallenge =
          bonusChallenges.firstWhere((e) => e.id == 'challenge_100_pushups');
      expect(pushupChallenge.xpReward, 100);

      final plankChallenge =
          bonusChallenges.firstWhere((e) => e.id == 'challenge_plank_5min');
      expect(plankChallenge.xpReward, 80);

      final burpeeChallenge =
          bonusChallenges.firstWhere((e) => e.id == 'challenge_burpee_ladder');
      expect(burpeeChallenge.xpReward, 120);
    });

    test('Mobility & recovery extras have appropriate properties', () async {
      final source = LocalExtrasSource();
      final mobilityExtras =
          await source.getExtrasByType(ExtraType.mobilityRecovery);
      ...
      final hipFlow =
          mobilityExtras.firstWhere((e) => e.id == 'mobility_hip_flow');
    ...
    test('Express workouts have varied difficulty levels', () async {
      final source = LocalExtrasSource();
      final expressWorkouts =
          await source.getExtrasByType(ExtraType.expressWorkout);
      ...
      final hardWorkouts =
          expressWorkouts.where((e) => e.difficulty == 'hard').toList();
    ...
    test('Extras data is properly structured', () async {
      ...
      for (final extra in extras) {
        ...
        expect([
          ExtraType.expressWorkout,
          ExtraType.bonusChallenge,
          ExtraType.mobilityRecovery
        ], contains(extra.type));
      }
    });
  });
