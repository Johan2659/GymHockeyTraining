...
void main() {
  group('Step 9 — Persistence Integration Tests', () {
    ...
    test('Critical fields are saved (role, cycle, sessions, streaks, XP)',
        () async {
      ...
      expect(appState.state?.completedExerciseIds,
          isNotEmpty); // Completed sessions
    ...
    test('Persistence handles graceful fallback', () async {
      ...
      final writeSuccess = await PersistenceService.writeWithFallback(
          'test_box', 'test_key', testValue);
      expect(writeSuccess, isTrue);

      final readValue =
          await PersistenceService.readWithFallback('test_box', 'test_key');
