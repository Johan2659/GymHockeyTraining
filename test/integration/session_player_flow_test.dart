  ...
  group('Integration Tests - SessionPlayer Flow', () {
    ...
    group('Complete SessionPlayer Flow', () {
      test('should complete full session flow from start to finish', () async {
        ...
        await container
            .read(markExerciseDoneActionProvider('sprint_30m').future);
        await container
            .read(markExerciseDoneActionProvider('agility_ladder').future);

        // Verify exercises were added
        final stateAfterExercises =
            await container.read(programStateProvider.future);
        expect(
            stateAfterExercises?.completedExerciseIds, contains('sprint_30m'));
        expect(stateAfterExercises?.completedExerciseIds,
            contains('agility_ladder'));
        ...
        final sessionStarted =
            events.where((e) => e.type == ProgressEventType.sessionStarted);
        final exercisesDone =
            events.where((e) => e.type == ProgressEventType.exerciseDone);
        final bonusDone =
            events.where((e) => e.type == ProgressEventType.bonusDone);
        final sessionCompleted =
            events.where((e) => e.type == ProgressEventType.sessionCompleted);
      ...
      test('should handle program pause and resume correctly', () async {
        ...
        expect(resumedState?.completedExerciseIds,
            contains('exercise1')); // Should retain progress
      });
      ...
      test('should handle multiple sessions progression', () async {
        ...
        await container
            .read(markExerciseDoneActionProvider('exercise2').future);
        await container.read(completeSessionActionProvider.future);
    ...
    group('Error Handling and Edge Cases', () {
      test('should handle completing exercises without active program',
          () async {
