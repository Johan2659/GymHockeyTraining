  ...
  group('Hockey Training System Tests', () {
    ...
    group('Exercise Database Tests', () {
      ...
      test('should load specific exercises by ID', () async {
        final backSquat = await HockeyExercisesDatabase.getExerciseById('back_squat');
      ...
      test('should return null for non-existent exercise', () async {
        final exercise = await HockeyExercisesDatabase.getExerciseById('non_existent_exercise');
        
        expect(exercise, isNull);
      });

      test('should filter exercises by category', () async {
        final strengthExercises = await HockeyExercisesDatabase.getExercisesByCategory(ExerciseCategory.strength);
        final powerExercises = await HockeyExercisesDatabase.getExercisesByCategory(ExerciseCategory.power);
        final conditioningExercises = await HockeyExercisesDatabase.getExercisesByCategory(ExerciseCategory.conditioning);
      ...
      test('should search exercises by name and category', () async {
        final squatExercises = await HockeyExercisesDatabase.searchExercises('squat');
        final strengthResults = await HockeyExercisesDatabase.searchExercises('strength');
        
        expect(squatExercises, isNotEmpty);
        expect(strengthResults, isNotEmpty);
        
        // Verify squat exercises contain squat in name
        final hasSquatInName = squatExercises.any((e) => 
    ...
    group('Attacker Program Tests', () {
      ...
      test('should load specific session by ID', () async {
        final session = await AttackerProgramData.getSessionById('attacker_w1_s1');
        ...
        final hasExerciseBlocks = session.blocks.any((block) => 
          block.exerciseId.isNotEmpty);
        expect(hasExerciseBlocks, isTrue);
      });

      test('should return null for non-existent session', () async {
        final session = await AttackerProgramData.getSessionById('non_existent_session');
    ...
    group('Local Source Integration Tests', () {
      test('LocalExerciseSource should work with hockey database', () async {
        ...
        final strengthExercises = await source.getExercisesByCategory('strength');
        expect(strengthExercises, isNotEmpty);
      });
      ...
      test('LocalProgramSource should load attacker program', () async {
        ...
        final attackerProgram = await source.getProgramById('hockey_attacker_2025');
        ...
        final attackerPrograms = await source.getProgramsByRole(UserRole.attacker);
    ...
    group('Exercise Alternatives Tests', () {
      test('should have proper gym and home alternatives', () async {
        ...
        final gymAlt = await HockeyExercisesDatabase.getExerciseById(backSquat.gymAltId!);
        final homeAlt = await HockeyExercisesDatabase.getExerciseById(backSquat.homeAltId!);
      ...
      test('should have YouTube queries for all exercises', () async {
        ...
        for (final exercise in exercises.take(10)) { // Test first 10
    ...
    group('Data Consistency Tests', () {
          ...
          for (final block in session.blocks) {
            if (block.exerciseId.isNotEmpty) {
              expect(exerciseIds, contains(block.exerciseId),
                reason: 'Exercise ${block.exerciseId} not found in database for session ${session.id}');
            }
          }
        }
      });
      ...
      test('alternative exercise IDs should exist in database', () async {
        ...
        for (final exercise in exercises.take(20)) { // Test first 20
          if (exercise.gymAltId != null) {
            expect(exerciseIds, contains(exercise.gymAltId),
              reason: 'Gym alternative ${exercise.gymAltId} not found for ${exercise.id}');
          }
          
          if (exercise.homeAltId != null) {
            expect(exerciseIds, contains(exercise.homeAltId),
              reason: 'Home alternative ${exercise.homeAltId} not found for ${exercise.id}');
          }
        }
      });
    });
